...
> **还能自己定义菜品** —— 拍一张你正在纠结的东西，它立刻变成一张立在盘子里的照片卡，
> 一起参与转动；自定义的菜可以改、可以删、可以导出备份，还能压进一条 base64 链接分享给朋友。

**▶️ 在线试玩：<https://hencter.github.io/chishenme-3d/>**（手机打开效果最好）
...
除了拍照，也可以「手动加一道」；自己加的菜随时能改（点菜单里的 ✏️）或删。
所有自定义菜都存在浏览器里：元数据进 localStorage，照片进 IndexedDB，**不上传任何服务器**。
在「管理我的菜品」里还能：

- **导出文件** —— 生成一份 JSON 备份（照片内嵌成 dataURL），换设备 / 换浏览器时导入恢复；
- **导入文件** —— 合并式导入：同 id 的菜更新，新 id 的菜加入，不会把现有菜单弄丢；
- **分享链接** —— 把整桌自定义菜压进一条 **base64 链接**（`#share=...`）。
  朋友打开链接就会看到「收到一份菜单分享」，点一下收进自己的星盘。
  链接里的照片会自动压成 360px 小图（一道菜通常几 KB），不想带照片也可以取消勾选。

分享数据放在 URL 的 `#hash` 里，**不会随请求发到任何服务器** —— 链接本身就是数据。
...
| `pnpm test` | **无浏览器**冒烟测试（521 项） |
| `pnpm check` | **真实浏览器**验收（120 项，会自动截图到 `shots/`） |
...
│   ├── custom.js           自定义菜：照片压缩、主色调提取、IndexedDB 存储、增删改导入
│   └── transfer.js         导入导出 / base64 分享链接：payload 校验与编解码（纯函数）
├── ui/
...
8. 加菜抽屉的入场动画写成 `translateY(100%)`，动画一旦卡住**整个表单被推到屏幕外**。

第 2 条和第 8 条是同一类问题，也是这个项目最值钱的一条经验：

> **入场动画的「起始态」必须本身可见、可点。**
> 动画是锦上添花，不能是元素可用的前提 —— 它随时可能因为主线程卡顿而不推进。
> 所以 `@keyframes` 不要从 `opacity: 0` / `translateY(100%)` / `visibility: hidden` 起步，
> 从「稍微偏一点、稍微淡一点」起步就够了。
